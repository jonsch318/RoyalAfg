...
import { rW, rH, registerApp } from "./utils";
...
class View extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.game.started.then(() => {
            ...
            PIXI.Loader.shared.add("textures/cards.json").load(this.setup.bind(this));
            ...
            this.board.update({
                updatedWidth: () => {
                    this.board.position.set(
                        this.app.renderer.width / 2 - this.board.width / 2,
                        this.app.renderer.height / 2 - this.board.height / 2
                    );
                }
            });
            this.board.position.set(
    ...
    setup() {
        this.didSetup = true;
        this.id = PIXI.Loader.shared.resources["textures/cards.json"].textures;
    ...
    render() {
        return (
            <div id="view">
                {!this.state.loaded ? <h1>Waiting for next game to start.</h1> : <></>}
            </div>
        );
    }
}
